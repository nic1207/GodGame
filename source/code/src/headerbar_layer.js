...
var HeaderBarLayer = cc.LayerGradient.extend({
  ...
  ctor:function (level) {
    ...
    //this.setItemCharacterStatus(this.gameConfig.items.butter.texture);
  },
  ...
  createHeaderMenus: function() {
    ...
    var menuItemPause = cc.MenuItemSprite.create(menuConfig.pauseNormal, menuConfig.pauseSelected,
      this.onGoPause, this);
    menuItemPause.setPosition(-20, 20);
    menuItemPause.setScale(0.5);
  ...
  setAbcCharacterStatus: function(string,index) { 
    var abcStatusConfig = this.gameConfig.headerbar.abcStatus;

    //不能放超過7個
    if(index > abcStatusConfig.num)
      return;

    var spriteFrameCache = cc.SpriteFrameCache.getInstance(); //使用精灵帧缓存，方便后面多次使用
    //第一个参数plist文件，第二个参数plist对应的png图片  
    var frameCache = spriteFrameCache.addSpriteFrames(s_ABCAlphabetPlist, s_ABCAlphabet);
    var str = string + ".png";
    var abcIcon = cc.Sprite.createWithSpriteFrame(spriteFrameCache.getSpriteFrame(str));

    abcIcon.setAnchorPoint(abcStatusConfig.textureAnchorPoint) ;
    abcIcon.setScale(abcStatusConfig.textureSize);
    abcIcon.setPosition(cc.p(this.size.width/2 - 100 + index * 25, 7));
    this.addChild(abcIcon);

    return abcIcon;
  },

  setItemStatus: function(config,index) { 
    var itemIcon = new cc.Sprite(config.texture, config.textureRect) ;
    itemIcon.setAnchorPoint(cc.p(0,0)) ;
    itemIcon.setScale(config.textureSize);
    itemIcon.setColor(cc.c3b(255, 255, 112));
    itemIcon.setPosition(cc.p(this.size.width/2 - 100 + index * 25, 7));
